...
namespace EShoppy.User_Module.Implementation
{
    public class ClientManager : IClientManager
    {
        ...
        public void AddFunds(IClient client, Guid accountID, double amount, ICurrency currency)
        {
            ...
            {
                throw new Exception("Iznos mora biti veci 10000 din.");
            }
            ...
            {
                throw new KeyNotFoundException("Korisnicki nalog nije nadjen u bazi.");
            }
            var dbAccount = financeManager.GetAccountByID(accountID);

            if(dbAccount == null)
            {
                throw new KeyNotFoundException("Racun nije nadjen u bazi.");
            }
            
            if(clientAccount.CreditAvailable)
            {
                financeManager.CreditPayment(accountID, amount);
                emailSender.SendEmail(client.Email, "Rata kredita uspesno placena.", "Rata za kredit uspesno placena.\nIznos koji je uplacen je: " + amount + "\nHvala vam sto koristite usluge nase banke.");
            }
            else
            {
